# <code>ervx <b>unlock</b></code>

## 1. About 
The `ervx` executable is composed of several inter-related sub commands. Please see `ervx -h` for all available options.

This part of the documentation describes options and concepts for <code>ervx <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking ervx pipeline output directory is fast and easy! In its most basic form, <code>ervx <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./ervx unlock [-h] --output OUTPUT
```
...
ervx unlock --output /data/$USER/output
```